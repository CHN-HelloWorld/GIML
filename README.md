...
<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/version-2.4.4-blue.svg" alt="Version">
...
### 首次配置
> 新安装XXMI的用户需要先用XXMI启动一次游戏，让GIMI初始化后方可继续使用GIML
...
**启用/禁用**：在非编辑模式下，可直接点击插件项旁的按钮实现**热切换（启用/禁用）**。被禁用的插件无需从列表中彻底删除，后续可随时启用。
...
- **路径**：⚙️ 设置 → 🎨 主题
- **内置主题**：内置多套精美角色主题，会随设定时间全自动无缝切换。
- **自定义主题**：勾选后可自由选择过往的预设主题，不再根据时间段自动切换，并支持开启“随机主题”模式。
- **自定义背景图**：勾选后可单独自定义“主页背景图”与“设置页背景图”，并可额外指定要使用的主题配色。**支持选择单张图片或整个图片文件夹**，当指定为文件夹时，还可设置“随机更换”或“按名称顺序更换”模式。
...
<details>
<summary><b>总是提示GIML违规通告</b></summary>

出现此提示说明系统检测到了正在运行的直播推流软件（如OBS、直播姬等）。为了维护Mod生态和遵守相关规定，本启动器严禁在直播环境下使用。
请彻底后台关闭所有直播推流软件及相关进程后，再重新使用GIML启动游戏。
</details>
