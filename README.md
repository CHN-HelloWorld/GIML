...
- **帧率自定义**：自由设置目标帧率（最高120FPS）
- **HDR支持**：支持HDR显示模式
- **多显示器支持**：可选择任意显示器启动游戏
- **自动分辨率适配**：根据窗口模式和显示器自动调整分辨率
- **智能限制**：自动限制帧率上限为120，确保稳定运行

> 📌 **帧率说明**：由于游戏内部限制，帧率最高只能设置为120FPS。超出此数值会被自动调整为120。
...
5. **高级设置（可选）**
   ...
   - **基础设置**：DLL版本选择、手动启动游戏选项
   - **帧率与画面设置**：目标帧率（最高120FPS）、HDR、窗口模式、显示器选择等
   - 配置好后点击"保存"按钮
...
4. **帧率限制**：FPS最高只能设置为120，这是由游戏内部机制决定的，超出此数值会被自动调整
...
**Q: FPS解锁器如何使用？**  
A: 在"设置"→"帧率与画面"中勾选"启用帧率与画面设置"，然后配置目标帧率和窗口模式等选项。帧率最高可设置为120FPS。

**Q: 为什么帧率最高只能设置为120？**  
A: 这是由游戏内部机制限制的，超过120FPS可能导致游戏不稳定。程序会自动将超出的数值调整为120以确保稳定运行。
...
- **稳定解帧**：内置FPS解锁器（最高120FPS），多种窗口模式，支持多显示器
