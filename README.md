# 为维护公平的游戏环境，所有过往程序包均已删除，暂不提供下载，待完善功能限制后会重新发布，预计在2025.11.22之前完成
# 原神Mod启动器 (GIML)
...
- **帧率限制提升**：帧率上限从120提升至480，适配高刷新率显示器

> 📌 **帧率说明**：帧率最高可设置为480FPS，建议根据显示器刷新率和电脑性能合理设置。过高帧率可能导致游戏不稳定或功耗增加。
...
4. **帧率建议**：帧率最高可设置为480FPS，但建议根据您的显示器刷新率合理设置
5. **多显示器支持**：支持显示器热插拔，如果保存的显示器配置失效（如拔掉第二显示器），程序会自动切换到主显示器并使用合适的分辨率
...
**Q: FPS解锁器如何使用？**  
A: 在"设置"→"帧率与画面"中勾选"启用帧率与画面设置"，然后配置目标帧率和窗口模式等选项。帧率最高可设置为480FPS。

**Q: 帧率应该设置多少合适？**  
A: 建议根据您的显示器刷新率设置，如60Hz显示器设置60FPS，144Hz设置144FPS，240Hz设置240FPS。过高帧率会增加功耗且可能影响稳定性。
...
- **强大解帧**：内置FPS解锁器（最高480FPS），多种窗口模式，智能多显示器支持
